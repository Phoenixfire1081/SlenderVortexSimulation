...
# Here, _step == 0 is still handled by python itself. Afterwards, the 
# FORTRAN code takes over.
# NOTE: partial FORTRAN only computes the uttm part of the code. Time
# integration is still handled by python
# NOTE #2: FORTRAN is meant only for the M1 KK method. For LIA, use python
...
_uniform, _shear, _BL, Ub, yShear, maxV, yPlus, uVelBL, vVelBL, wVelBL, _image):
	...
	if not _LIA:
		...
		os.chdir('src/partial_fortran')
		os.system('./uttm_partial')
		vec_all = np.loadtxt('fout.dat')
		os.system('rm -rf fout.dat')
		os.chdir('../../')
